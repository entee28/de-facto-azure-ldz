locals {
  ...
  default_resource_group_name = format("rg-%s-001", local.name_suffix)
  default_virtual_wan_name    = format("vwan-%s-001", local.name_suffix)
  default_virtual_hub_name    = format("vhub-%s-001", local.name_suffix)
