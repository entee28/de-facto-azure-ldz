locals {
  ...
  name_suffix = format("connectivity-%s-%s", var.environment, local.location_code)
  ...
  default_dns_resolver_name   = format("dnspr-%s-001", local.name_suffix)
  ...
  virtual_network_resource_id = format(
    ...
    var.shared_services_virtual_network_name
  )
}
