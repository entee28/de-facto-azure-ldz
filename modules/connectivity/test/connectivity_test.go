...
func TestConnectivityLDZModule(t *testing.T) {
	...
	hubSubnets := map[string]HubSubnet{
		"dnspr-in": {
			Name:            "snet-dnspr-in-prd-sea-001",
			AddressPrefixes: "10.10.1.0/27",
	...
	userRouteTables := map[string]UserRouteTable{
		"rt-snet-test": {
			Name: "rt-snet-test",
		},
	}

	userNetworkSecurityGroups := map[string]UserNetworkSecurityGroup{
		"nsg-snet-test": {
			Name: "nsg-snet-test",
		},
	}
	...
	terraformOptions := &terraform.Options{
		...
		Vars: map[string]interface{}{
			...
			"route_table_entries_user_subnets": []RouteTableEntry{
				{
					Name:             "route-to-all",
			...
			"dnsresolver_inbound_endpoints": map[string]DNSResolverInboundEndpoint{
				"dnspr-inbound": {
					Name:       "in-dnspr-connectivity-prd-sea-001",
					SubnetName: "snet-dnspr-in-prd-sea-001",
				},
			},
			"dnsresolver_outbound_endpoints": map[string]DNSResolverOutboundEndpoint{
				"dnspr-outbound": {
					Name:       "out-dnspr-connectivity-prd-sea-001",
					SubnetName: "snet-dnspr-out-prd-sea-001",
					ForwardingRuleset: map[string]ForwardingRuleset{
						"frs-dnspr-outbound": {
